...
{
    if(("ajaxGameDetail" in window && packageName === ajaxGameDetail.resPackagename) || ("ajaxGiftDetail" in window && packageName === ajaxGiftDetail.resPackagename) || ("ajaxInfoDetail" in window && packageName === ajaxInfoDetail.resPackagename))
    {
        isUxbao && window.activity.update(JSON.stringify(state));
    }
    else
    {
        var items = document.all(packageName);
        if(items.length > 1){
...
function btnTapHandler($target)
{
    ...
    if($target.hasClass('dlBtn') || $target.hasClass('continueBtn'))
    {
        ...
        {
            if(userInfo.userState || "ajaxFreeGame" in window)
            {
                isUxbao && window.uxbao.addSaveDataFlow($item.data("capacity"));
            }
            if("ajaxWorldCup" in window)
            {
                ++ajaxWorldCup.count;
...
function bindGiftTapHandler($item, itemData)
{
    if(itemData.acId)
    {
        $item.find('.relate-gift').css("display", "block");
    }
    else
    {
        $item.find('.relate-gift').hide();
    }
}
...
{
    //详情
    if("ajaxGiftDetail" in window && acId == ajaxGiftDetail.acId)
    {
